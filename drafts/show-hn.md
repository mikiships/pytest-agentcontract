...
I built a pytest plugin that records an LLM agent's tool-call trajectory once, then replays it deterministically in CI. No API keys, no network, no flakes, sub-second runs.
...
The approach: treat the tool-call sequence as the agent's contract. Record it once against real APIs, save it as a cassette (.agentrun.json), then replay it offline forever.
...
77 tests, MIT license, zero config to start. Convention over configuration, but agentcontract.yml if you want fine-grained control.
