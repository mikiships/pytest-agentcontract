...
# Replay in CI forever (deterministic, no network if your test stays on recorded runs or tool stubs)
pytest --ac-replay
```
...
def test_with_real_agent(ac_recorder):
    ...
    # Each chat.completions.create response is recorded as an assistant turn.
    # Tool requests are captured automatically, but tool results are not.
...
The SDK interceptors only see model responses, so they record assistant content plus tool call requests. Tool results come from your application code in later turns and still need to be recorded manually or backfilled later.
