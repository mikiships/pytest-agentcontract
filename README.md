...
including nested dictionary keys and values. Supported categories are `email`, `phone` (US
formatted numbers), `ssn`, and `credit_card` (Luhn-valid card numbers only). Findings report only
masked snippets, never full raw PII values; locations for PII-bearing dictionary keys are also
masked. Use `block` to check specific categories; omit it or leave it empty to check all supported
categories.
