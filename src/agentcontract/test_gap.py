...
def analyze_test_gaps(
    ...
    ranked_modules = tuple(
        sorted(
            (module for module in all_modules if module.missing_line_count > 0),
            key=_gap_sort_key,
        )
    )
    return GapReport(
        ...
        analyzed_module_count=len(all_modules),
...
def _candidate_test_stems(relative_source_path: Path) -> frozenset[str]:
    ...
    parent_name = relative_source_path.parent.name
    if parent_name:
        candidates.add(f"test_{parent_name}")
        candidates.add(f"test_{stem}_{parent_name}")
        candidates.add(f"test_{parent_name}_{stem}")
        singular_parent = parent_name[:-1] if parent_name.endswith("s") else parent_name
        candidates.add(f"test_{stem}_{singular_parent}")
