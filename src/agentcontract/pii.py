...
class PIIFinding:
    ...
    def to_dict(self) -> dict[str, str | None]:
        """Serialize the finding without raw PII."""
        return {
...
class PIIScanResult:
    ...
    def to_dict(self) -> dict[str, object]:
        """Serialize the result without raw PII."""
        return {
...
def scan_payload(
    ...
    *,
    ...
    """Recursively scan dict/list/scalar payload values."""
    ...
    if isinstance(payload, dict):
        for key, value in payload.items():
            result.extend(
                _scan_payload(
                    value,
                    location=_join_location(location, str(key)),
                    cassette_path=cassette_path,
...
def _normalize_categories(categories: Iterable[str] | None) -> set[str]:
    ...
    if unknown:
        raise ValueError(f"Unsupported PII categories: {', '.join(unknown)}")
    return normalized
