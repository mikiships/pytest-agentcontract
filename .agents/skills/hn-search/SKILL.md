...
Call the API with the currently available web/open tool or a shell HTTP client such as:
...
Calculate the current Unix timestamp first (for example, `date +%s` in a shell), then subtract:
