...
compatibility: Requires network access and an HTTP-capable fetch mechanism such as curl or the agent environment's web tools.
---
...
Use the available HTTP fetch mechanism to call the API. When shell access is available, prefer `curl -fsSL` so the raw response can be parsed as JSON.
