...
Use the current agent's web or HTTP fetch capability, or run `curl` from the shell. The response is JSON.
...
Calculate the current Unix timestamp first (for example, `date +%s`), then subtract:
