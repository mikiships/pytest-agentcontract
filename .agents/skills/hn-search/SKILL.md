...
Use the current agent's HTTP-capable tool to call the API, such as a URL fetch/open tool or a shell `curl` request. Response is JSON.
...
Calculate the current Unix timestamp first (for example, with `date +%s`), then subtract:
