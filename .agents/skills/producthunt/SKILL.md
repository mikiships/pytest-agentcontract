...
If no token is available, fall back to the agent's available web search capability with `site:producthunt.com` queries.
...
**To find a product by name**, use the available web search capability first:
```
...
**Remember:** This browses a topic — it's not a text search. To find a specific product by name, use the available web search capability with `site:producthunt.com/posts "product name"`, then look up the post by slug via the API.
...
1. Use the agent's available web search capability with queries like:
   - `site:producthunt.com/posts "product name"`
   - `site:producthunt.com "topic" launched`
2. Fetch specific Product Hunt URLs with the available web fetch/browser capability, or use `curl` when a plain HTTP request is enough
...
- **Post not found:** The slug may be wrong. Try searching the web first to confirm the exact slug.
