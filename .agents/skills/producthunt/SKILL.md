...
compatibility: Requires network access. API-backed details require PH_API_TOKEN; basic lookup can fall back to web search.
---
...
If no token is available, fall back to the available web search capability with `site:producthunt.com` queries.
...
**To find a product by name**, use web search first:
```
...
Use `curl` from the shell to make GraphQL requests:
...
**Remember:** This browses a topic — it's not a text search. To find a specific product by name, use web search with `site:producthunt.com/posts "product name"`, then look up the post by slug via the API.
...
2. Open or fetch specific Product Hunt URLs to get basic info
...
1. Search: `site:producthunt.com/posts linear`
