...
If no token is available, fall back to the current agent environment's web search capability with `site:producthunt.com` queries.
...
**Remember:** This browses a topic — it's not a text search. To find a specific product by name, search the web with `site:producthunt.com/posts "product name"`, then look up the post by slug via the API.
