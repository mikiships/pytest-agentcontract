...
If no token is available, fall back to the currently available web search capability with `site:producthunt.com` queries.
...
Use a shell with `curl` to make GraphQL requests:
...
**Remember:** This browses a topic — it's not a text search. To find a specific product by name, search for `site:producthunt.com/posts "product name"`, then look up the post by slug via the API.
...
1. Use the available web search tool with queries like:
   - `site:producthunt.com/posts "product name"`
   - `site:producthunt.com "topic" launched`
2. Open specific Product Hunt URLs with the available browser/web-open tool to get basic info
