...
If no token is available, fall back to the active agent's web search capability with
`site:producthunt.com` queries.
...
1. Use the active agent's web search capability with queries like:
   - `site:producthunt.com/posts "product name"`
   - `site:producthunt.com "topic" launched`
2. Use the active agent's web fetch/open capability on specific Product Hunt URLs to get basic info
