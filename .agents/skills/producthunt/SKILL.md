...
If no token is available, fall back to `web.search_query` with `site:producthunt.com` queries.
...
**To find a product by name**, use `web.search_query` first:
```
...
- **Format:** GraphQL
- **Rate Limit:** GraphQL requests are complexity-limited. Product Hunt's docs currently describe a 6250-point budget per 15 minutes for `/v2/api/graphql`, with response headers exposing the remaining quota.

## How to Query

Use `exec_command` with `curl` to make GraphQL requests:
...
**Remember:** This browses a topic — it's not a text search. To find a specific product by name, use `web.search_query` with `site:producthunt.com/posts "product name"`, then look up the post by slug via the API.
...
1. Use `web.search_query` with queries like:
   - `site:producthunt.com/posts "product name"`
   - `site:producthunt.com "topic" launched`
2. Use `web.open` on specific Product Hunt URLs to get basic info
...
- **Post not found:** The slug may be wrong. Try searching with `web.search_query` first to confirm the exact slug.
...
1. Search with `web.search_query`: `site:producthunt.com/posts linear`
