...
Use the best available capture method for the surface being reviewed:

- For web UI, prefer available browser automation such as Playwright or the agent's browser screenshot capability.
- For local browser targets, capture the actual rendered page at the requested viewport size.
- For desktop applications or system UI, use an OS-level screenshot only when browser automation is not appropriate.
- User-provided screenshots are acceptable; inspect the attached image directly.

Save the screenshot path or keep the attached image available for the review step.

### Step 2: Analyze with Vision

Use the agent's image-viewing or vision capability to analyze the screenshot:

```
[path or URL to screenshot]
...
Use these prompts with the available image-viewing or vision capability depending on what you need:
