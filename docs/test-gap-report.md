...
| --- | --- | ---: | --- |
| P0 | `src/agentcontract/plugin.py` | 0% | Pytest plugin options, fixtures, replay/record paths, assertion merging |
| P1 | `src/agentcontract/cli.py` | 38% | Most command success and failure paths |
| P1 | `src/agentcontract/adapters/openai_agents.py` | 32% | Runner patching plus most extraction helpers |
| P2 | `src/agentcontract/__init__.py` | 33% | Lazy public API imports and unknown attribute path |
...
`tests/unit/test_cli.py` currently covers only the invalid-cassette path for `info`. The CLI should
get focused command-level tests because it is a public entry point and has several simple branches
that are cheap to cover.
...
- Async `Runner.run` is patched, records latency-backed turns, returns the original result, and is
  restored by `unpatch`.
- `_extract_from_result` prefers `new_items` over `final_output` and ignores empty/unsupported
  result shapes.
...
- Dict and object tool calls with non-dict `args`/`arguments` are coerced to `{}`.
...
- Add assertion engine tests for exception handling, unknown assertion types, unknown/unsupported
...
- Add serialization tests for save/load write failures, invalid top-level JSON, non-object decoded
  JSON, and non-serializable fallback branches that are not already covered.
