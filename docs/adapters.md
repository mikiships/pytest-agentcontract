...
Tool calls are extracted from message `tool_calls` entries using `name`, `args` or `arguments`, and `id`. Tool messages can be recorded as `tool` turns when they are present in the returned `messages` list, but the LangGraph adapter does not attach those outputs to the preceding `ToolCall.result`.
