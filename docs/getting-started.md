...
The adapter modules for LangGraph, LlamaIndex, and the OpenAI Agents SDK are included in the package. Your application still needs to install the framework it uses. The OpenAI Agents SDK adapter imports the `agents` package at runtime, so install `openai-agents` separately when using that adapter.
