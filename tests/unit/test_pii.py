...
def test_scan_payload_rejects_unknown_categories_without_raw_pii_leak() -> None:
    with pytest.raises(ValueError) as exc_info:
        scan_payload("safe", categories=["alice@example.com"])

    message = str(exc_info.value)
    assert "Unsupported PII categories" in message
        ...
        turns=[
            ...
            Turn(
                ...
                tool_calls=[
                    ToolCall(
                        id="tc1",
                        function="notify_alice@example.com",
                        arguments={
                            "customer": {"email": "alice@example.com"},
