...
def test_find_test_gaps_does_not_flag_module_with_package_test_importing_it(tmp_path: Path) -> None:
    repo = _make_repo(tmp_path)
    _write(repo / "src/agentcontract/recorder/__init__.py", "")
    _write(
        repo / "src/agentcontract/recorder/core.py",
        "class Recorder:\n    pass\n",
    )
    _write(
        repo / "src/agentcontract/recorder/interceptors.py",
    ...
    _write(
        repo / "tests/unit/test_recorder.py",
    ...
    weak_modules = {gap.module_name for gap in report.weak}
    assert "agentcontract.recorder.core" not in weak_modules
    assert "agentcontract.recorder.interceptors" in weak_modules
